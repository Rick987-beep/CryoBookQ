"""Live ME2: Binance eoptions 100-symbol burst (fstream + REST fill)."""
...
@pytest.mark.asyncio
async def test_me2_binance_burst() -> None:
    require_network()
    venue = BinanceVenue()
    ...
    assert stats.coverage >= 0.80, (
        f"Binance coverage {stats.coverage:.2%} notes={stats.notes} errors={stats.subscribe_errors[:3]}"
    )
